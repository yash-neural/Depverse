...
class CliChat(Chat):
    ...
    async def _extract_resources(self, query: str) -> str:
        # Short-circuit: no "@" in the query means no doc mentions, so we
        # don't need to hit the server for the docs://documents resource
        # (which only exists when the server provides one).
        mentions = [word[1:] for word in query.split() if word.startswith("@")]
        if not mentions:
            return ""

        # Fetching the doc list can fail if the current MCP server doesn't
        # expose it (e.g. the npm server). Treat that as "no docs available".
        try:
