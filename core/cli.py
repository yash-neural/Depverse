# Standard typing helpers — List[str], Optional[X] make function signatures self-documenting.
from typing import List, Optional

# PromptSession: the main interactive input object from prompt-toolkit.
# It draws the "> " prompt, reads keystrokes, and supports async via prompt_async().
from prompt_toolkit import PromptSession

# Completer  : base class we subclass to produce auto-complete suggestions (e.g. doc IDs after "@").
# Completion : the value object we yield from the completer — one suggestion with its label and metadata.
from prompt_toolkit.completion import Completer, Completion

# KeyBindings: lets us bind keys (like "/", "@", space) to custom actions that trigger completions.
from prompt_toolkit.key_binding import KeyBindings

# Style: theming for the prompt and completion menu (colors, background, etc.).
from prompt_toolkit.styles import Style

# InMemoryHistory: stores past inputs so ↑/↓ arrow keys navigate command history within a session.
from prompt_toolkit.history import InMemoryHistory

# AutoSuggest: shows a faint inline hint ahead of the cursor (like fish/zsh suggestions).
# Suggestion : the value object that describes the hint text to display.
from prompt_toolkit.auto_suggest import AutoSuggest, Suggestion

# Document: a read-only snapshot of buffer text + cursor position — used inside completers.
from prompt_toolkit.document import Document

# Buffer: the mutable text buffer behind the prompt — used inside key bindings to insert text/trigger completion.
